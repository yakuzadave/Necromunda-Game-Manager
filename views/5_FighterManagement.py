...
if selected_gang.gangers:
    fighter_options = {fighter.name: fighter for fighter in selected_gang.gangers}
    selected_fighter_name = st.selectbox("Select a Fighter", list(fighter_options.keys()))
...
editable_fields = {
    ...
    "type": selected_fighter.type,
    "m": selected_fighter.m,
    "ws": selected_fighter.ws,
    "bs": selected_fighter.bs,
    "s": selected_fighter.s,
    "t": selected_fighter.t,
    "w": selected_fighter.w,
    "i": selected_fighter.i,
    "a": selected_fighter.a,
    "ld": selected_fighter.ld,
    "cl": selected_fighter.cl,
    "wil": selected_fighter.wil,
    "intelligence": selected_fighter.intelligence,
    "cost": selected_fighter.cost,
    "xp": selected_fighter.xp,
    "kills": selected_fighter.kills,
    "advance_count": selected_fighter.advance_count,
    "status": selected_fighter.status,
    "notes": selected_fighter.notes,
    # For equipment and injuries, join list elements into a comma-separated string.
    "equipment": ", ".join(eq.name for eq in selected_fighter.equipment) if selected_fighter.equipment else "",
    "injuries": ", ".join(selected_fighter.injuries) if selected_fighter.injuries else "",
}
...
if st.button("Save Changes"):
    ...
    for field in [
        "name", "type", "m", "ws", "bs", "s", "t", "w", "i", "a",
        "ld", "cl", "wil", "intelligence", "cost", "xp", "kills",
    ...
    if updated_values.get("equipment", "").strip():
        equipment_names = [name.strip() for name in updated_values["equipment"].split(",") if name.strip()]
        ...
        selected_fighter.equipment = [Equipment(name=name, qty=1) for name in equipment_names]
    else:
        selected_fighter.equipment = []

    if updated_values.get("injuries", "").strip():
        injuries_list = [inj.strip() for inj in updated_values["injuries"].split(",") if inj.strip()]
        selected_fighter.injuries = injuries_list
    else:
        selected_fighter.injuries = []

    # Update last updated timestamp
    selected_fighter.datetime_updated = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
st.markdown(f"**M:** {selected_fighter.m} | **WS:** {selected_fighter.ws} | **BS:** {selected_fighter.bs}")
st.markdown(f"**S:** {selected_fighter.s} | **T:** {selected_fighter.t} | **W:** {selected_fighter.w}")
st.markdown(f"**I:** {selected_fighter.i} | **A:** {selected_fighter.a}")
st.markdown(f"**Ld:** {selected_fighter.ld} | **Cl:** {selected_fighter.cl} | **Wil:** {selected_fighter.wil}")
st.markdown(f"**Int:** {selected_fighter.intelligence}")
st.markdown(f"**Cost:** {selected_fighter.cost} | **XP:** {selected_fighter.xp}")
st.markdown(f"**Kills:** {selected_fighter.kills} | **Advance Count:** {selected_fighter.advance_count}")
st.markdown("---")
