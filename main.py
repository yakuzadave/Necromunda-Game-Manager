...
# -------------------- File Names --------------------
DATA_FILE = "campaign_data.json"             # Local campaign data (gangs, territories, battles)
FULL_CAMPAIGN_DATA_FILE = "full_campaign_data.json"  # (Not auto-created; used when loading external full campaign data)
...
if 'gangs' not in st.session_state or 'territories' not in st.session_state or 'battles' not in st.session_state:
...
def export_campaign():
    ...
    if submit_export:
        ...
        # Store in session_state to display the download button outside the form
        st.session_state.full_campaign_json = full_campaign_json
        st.success("Full campaign JSON generated!")

    # Display the download button if JSON exists (outside the form)
